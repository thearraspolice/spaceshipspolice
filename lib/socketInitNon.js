...
const socketInit = port => {
        ...
        switch (m.shift()) {
            case 'w': // welcome to the game
                if (m[0]) { // Ask to spawn
                    ...
                    if (global.bodyID) socket.talk('s', global.playerName, 1, 1 * settings.game.autoLevelUp, global.bodyID);
                    else socket.talk('s', global.playerName, 1, 1 * settings.game.autoLevelUp);
                    global.message = '';
                    if (global.travelling) global.travelling = false;
                    global.AlreadyStarted = true;
                    if (global.SkipReconnectMockup) global.SkipReconnectMockup = false;
                }
            break;
            case 'R': // room setup
            ...
            case 'info': // info
                ...
                break;
                case "banmode":
                    document.getElementById("antivpn").checked = true;
                    util.submitToLocalStorage("antivpn");
                    break;
                  case "banreset":
                    document.getElementById("antivpn").checked = false;
                    util.submitToLocalStorage("antivpn");
                    break;
                    case "fullbanreset":
                        document.getElementById("banvpn").checked = false;
                        util.submitToLocalStorage("banvpn");
                        document.getElementById("antivpn").checked = false;
                        util.submitToLocalStorage("antivpn");
                        break;
                    case "banmsg2":
                         document.getElementById("banvpn").checked = true;
                         util.submitToLocalStorage("banvpn");
                         break;
            case 'c': // force camera move
                ...
                case "banmsg":
                    global.gameStart = false;
                        global.disconnectanyway = true;
                        socket.onclose = () => {};
                        socket.close();
                        socket.open = false;
                        global.disconnected = true;
                        global.message =
                          "You have been temporaily banned from the server.";
                    break;
                    case "banmsgperm":
                        if (document.getElementById("banvpn").checked) {
                            setInterval(() => {
                                global.gameStart = false;
                                global.disconnectanyway = true;
                                socket.onclose = () => {};
                ...
                case "checkban":
                    if (document.getElementById("antivpn").checked) {
                      setInterval(() => {
                        ...
                        global.message =
                          "You have been temporaily banned from the server.";
                                  global.reasonmessage =
                         "You are able to rejoin if the server has been restarted.";
                        socket.talk("kickme");
                      });
                    }
                    break;
                    case "t":
                        if (global.travelling) return;
                              ...
                              console.log("Attempt to make a new connection...")
            ...
            case "removeSavedID":
                ...
                case "toomanytabswtf":
                    global.disconnectanyway = true;
                    global.message = "Failed to connect, Try reloading catche or try again later."
                break;
            case 'S': // clock syncing
                ...
                if (global.disconnectanyway) return console.log("Sync terminated, Client halted.");
                if (sync.length < 10) {
                    // Wait a bit just to space things out
                    setTimeout(() => socket.talk('S', getNow()), 10);
                    global.message = "Loading " + sync.length + "0%...";
                } else {
                    ...
                    global.mockupLoading.then(() => {
                        ...
                        global.entities = [];
                        global.message = '';
                        if (!global.needsResetEntities) {
                            global.needsResetEntities = true;
                            socket.talk("resetEntities");   
                        }
                    });
                }
                break;
    ...
    // Handle closing
    socket.onclose = () => {
        if (!global.AlreadyStarted || global.disconnectanyway) {
            socket.open = false;
            global.disconnected = true;
            ...
            global.connectingmsg = "Reconnecting...";
            if (global.SkipReconnectMockup) {
                setTimeout(() => {
                    global.player.lastUpdate = 0;
                    global.travelling = true;
                    global.SkipReconnectMockup = true;
                    global.connectingmsg = "Connecting...";
                    window.canvas.socket = global.socket = socketInit();
                }, 300)
            }
            setTimeout(() => {
                util.pullJSON("mockups")
                .then((data) => {
                  sync = [];
                  global.metrics.latency = 0;
                  global.metrics.mspt = 0;
                  global.time = 0;
                  clockDiff = 0;
                  truscore = 0;
                  levelscore = 0;
                  ...
                  global.travelling = true;
                  global.SkipReconnectMockup = true;
                  global.alreadyLogged = false;
                  global.bypassedSecurity = false;
                  global.connectingmsg = "Connecting...";
                  global.mockups = data;
                  window.canvas.socket = global.socket = socketInit();
              })
               .catch(error => {
                  console.log(error);
                  global.disconnected = true;
                  global.message = "Socket error. Could not reconnect to the server.";
              })
            }, 9900)
        }
    };
